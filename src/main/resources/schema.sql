...
create table if not exists show_details (
    ...
    CONSTRAINT UC_show_details UNIQUE (show_date,start_time,event_id,event_place_id)
);
...
create table if not exists booking (
    ...
    show_details_id bigint not null,
    seat_number varchar(255) not null,
    is_active bool not null,
    
    CONSTRAINT UC_BOOKING UNIQUE (show_details_id,seat_number),
    CONSTRAINT FK_BOOKING_USER FOREIGN KEY(user_info_id) REFERENCES user_info(id),
    CONSTRAINT FK_BOOKING_show_details FOREIGN KEY(show_details_id) REFERENCES show_details(id)
);